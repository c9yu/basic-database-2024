...
- 데이터베이스 모델링
    ...
    <!-- md 주석. ![인덱스](https://raw.githubusercontent.com/c9yu/basic-database-2024/main/imamges/db009.png) -->
    <!-- html img 태그가 이미지 사이즈 조정 가능-->

    <img src = "https://raw.githubusercontent.com/c9yu/basic-database-2024/main/imamges/db009.png" width="900">
...
- 인덱스 예제
- 정규화
    - DB 상에서 생기는 이상현상(삽입, 삭제, 수정)이 생기지 않는 릴레이션(테이블)을 분리해서 데이터베이스를 설계한다.
    - 이상현상이 생기는 테이블을 분리해서 해결
    - 기본키와 함수 종속성을 파악
        - 함수 종속성의 개념
            - 학생수강성적 릴레이션에 각 속성 사이에는 의존성이 존재함
            - 어떤 속성 A의 값을 알면
            - 제 1정규형 : 도메인이 원자값을 가진다.
            - 제 2정규형 : 기본키가 아닌 속성이 기본키에 완전 종속일 때(학생번호[PK], 강좌이름 -> 성적을 결정)
            - 제 3정규형 : 기본키가 아닌 속성이 기본키에 비이행적으로 종속할 때 (학생번호 --> 강좌이름 --> 수강료 [이행종속성])
            - BCNF 정규형 : 함수 종속성 X -> Y가 성립할 때 모든 결정자가 X가 후보키 -> 속성이면
            - 보통 BCNF까지 정규형을 한다.
            - 4정규형(다치 종속성), 5정규형(조인 종속성, 무손실 분해)은 잘 하지 않는다.
- 실무 실습
    1. DB관리자 (SSMS)
        - hr 데이터베이스 생성, 관계를 설정
        - hr DB를 사용할 사용자 계정을 생성, 필요한 조건
            - 사용자계정 : hr_user, 비밀번호 : hr_p@ss!
            - 만약 sa 의 비밀번호를 잊어버리는 경우
                - 윈도우즈 인증으로 로그인 후, 보안 > sa > 속성에서 비밀번호 변경 이후 SQL Server 인증으로 로그인 하면 된다.
